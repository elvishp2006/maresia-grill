...
function RankingList({
  ...
  if (items.length === 0) {
    return <p className="text-[13px] leading-[1.5] text-[var(--text-dim)]">{emptyText}</p>;
  }
        ...
        <li key={item.id} className="rounded-[18px] border border-[var(--border)] bg-[var(--bg-card)] px-[12px] py-[10px]">
          <div className="flex items-start justify-between gap-[12px]">
            <div className="min-w-0">
              <p className="truncate text-[13px] font-semibold text-[var(--text)]">{item.nome}</p>
  ...
  return (
    <section className="mb-[18px] flex flex-col gap-[16px]">
      <div className="section-card">
        <div className="flex items-start justify-between gap-[12px]">
          <div>
            ...
            <h2 className="mt-[4px] font-[var(--font-display)] text-[22px] font-bold text-[var(--text)]">
              O que faz sentido hoje
            </h2>
            <p className="mt-[8px] max-w-[48ch] text-[13px] leading-[1.6] text-[var(--text-dim)]">
        ...
        {loading ? (
          <p className="mt-[14px] text-[13px] text-[var(--text-dim)]">Analisando seu histórico...</p>
        ) : error ? (
          <p className="mt-[14px] text-[13px] text-[var(--accent-red)]">{error}</p>
        ) : suggestedItems.length === 0 ? (
          <p className="mt-[14px] text-[13px] text-[var(--text-dim)]">
              ...
              <li key={item.id} className="rounded-[20px] border border-[var(--border)] bg-[var(--bg-card)] px-[14px] py-[14px] shadow-[0_8px_18px_rgba(0,0,0,0.08)]">
                <div className="flex items-start justify-between gap-[12px]">
                  <div className="min-w-0">
                    ...
                    <p className="mt-[2px] text-[12px] text-[var(--text-dim)]">{item.categoria}</p>
                    <p className="mt-[10px] text-[12px] leading-[1.6] text-[var(--text-dim)]">{item.reason}</p>
                  </div>
                  <button
                    type="button"
                    className="neon-gold-fill min-h-[42px] shrink-0 rounded-[16px] bg-[var(--accent)] px-[14px] py-[10px] text-[12px] font-semibold text-[var(--bg)] shadow-[0_8px_18px_rgba(0,0,0,0.12)] transition-opacity hover:opacity-90"
                    onClick={() => {
                      lightTap();
      ...
      <div className="section-card">
        ...
        {loading ? (
          <p className="mt-[14px] text-[13px] text-[var(--text-dim)]">Carregando estatísticas...</p>
        ) : error ? (
          <p className="mt-[14px] text-[13px] text-[var(--accent-red)]">{error}</p>
        ) : (
          <div className="mt-[16px] grid grid-cols-1 gap-[16px] lg:grid-cols-2">
            <div className="rounded-[22px] border border-[var(--border)] bg-[var(--bg-card)] p-[16px] shadow-[0_8px_20px_rgba(0,0,0,0.08)]">
              <h3 className="text-[15px] font-semibold text-[var(--text)]">Complementos mais usados</h3>
            ...
            <div className="rounded-[22px] border border-[var(--border)] bg-[var(--bg-card)] p-[16px] shadow-[0_8px_20px_rgba(0,0,0,0.08)]">
              <h3 className="text-[15px] font-semibold text-[var(--text)]">Mais usados por categoria</h3>
              <div className="mt-[12px]">
                <RankingList
            ...
            <div className="rounded-[22px] border border-[var(--border)] bg-[var(--bg-card)] p-[16px] shadow-[0_8px_20px_rgba(0,0,0,0.08)]">
              <h3 className="text-[15px] font-semibold text-[var(--text)]">Sequências recentes</h3>
              {streakItems.length === 0 ? (
                <p className="mt-[12px] text-[13px] text-[var(--text-dim)]">Sem sequências relevantes no período.</p>
              ) : (
                <ul className="mt-[12px] flex list-none flex-col gap-[10px]">
                  {streakItems.map(item => (
                    <li key={item.id} className="rounded-[18px] border border-[var(--border)] bg-[var(--bg-elevated)] px-[12px] py-[10px]">
                      <div className="flex items-center justify-between gap-[12px]">
                        ...
                        <span className="neon-gold-text shrink-0 text-[12px] font-semibold text-[var(--accent)]">{item.streak} dias</span>
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            <div className="rounded-[22px] border border-[var(--border)] bg-[var(--bg-card)] p-[16px] shadow-[0_8px_20px_rgba(0,0,0,0.08)] lg:col-span-2">
              <h3 className="text-[15px] font-semibold text-[var(--text)]">Complementos esquecidos</h3>
              {neglectedItems.length === 0 ? (
                <p className="mt-[12px] text-[13px] text-[var(--text-dim)]">Todos os itens tiveram alguma aparição recente.</p>
              ) : (
                <ul className="mt-[12px] flex list-none flex-col gap-[10px]">
                  {neglectedItems.map(item => (
                    <li key={item.id} className="flex items-center justify-between gap-[12px] rounded-[16px] border border-[var(--border)] px-[12px] py-[10px]">
                      ...
                      <span className="neon-gold-text shrink-0 text-[12px] font-semibold text-[var(--accent)]">{item.count}x</span>
                    </li>
                  ))}
                </ul>
              )}
